...
### 1b.
We now include the thorax variable, which is a numerical variable. That means we now have a factor variable, a numerical variable and a numerical outcome, which means that an ANCOVA-test is appropriate.

```{r}
fruitflies_model2 <- aov(loglongevity ~ activity + thorax, data=fruitflies)
fruitflies_model2
#Anova(fruitflies_model2, type="III") 
...
If we take the maximum and minimum thorax lenghts, we get the following thorax lengths and longevities:
...
### 1c.
We once again plot the longevity agains the thorax length:
```{r}
plot(high$thorax, high$longevity, col='blue', pch=20)
points(low$thorax, low$longevity, col='red', pch=20)
points(isolated$thorax, isolated$longevity, col='green', pch=20)
...
```{r}
fruitflies_model2 <- aov(longevity ~ activity + thorax, data=fruitflies)
fruitflies_model2
#Anova(fruitflies_model2, type="III") 
summary(fruitflies_model2)
```
